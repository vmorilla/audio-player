...
void set_tilemap_offset(uint16_t x, uint8_t y)
{
    ...
    ZXN_NEXTREGA(REG_TILEMAP_OFFSET_X_LSB, x & 0xff);
...
{
    ZXN_NEXTREGA(REG_SPRITE_NUMBER, index);
}
...
{
    ZXN_NEXTREGA(REG_SPRITE_ATTR_X, x & 0xff);
    ZXN_NEXTREGA(REG_SPRITE_ATTR_Y, y & 0xff);
...
{
    ZXN_NEXTREGA(REG_SPRITE_ATTR_X, x_offset);
    ZXN_NEXTREGA(REG_SPRITE_ATTR_Y, y_offset);
